...
async def recommend_jobs(
    ...
    """基于用户画像推荐岗位。"""
    return ApiResponse(data=await job_service.recommendations(session, page=1, page_size=limit))
...
async def get_job(
    ...
    """岗位详情。"""
    return ApiResponse(data=await job_service.detail(session, job_id))
...
async def match_job(
    ...
    """AI 分析岗位与用户简历的匹配度。"""
    return ApiResponse(data=await job_service.match(session, job_id, payload.resume_id))
